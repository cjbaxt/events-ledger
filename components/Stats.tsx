...
function BreakdownTab({ events, onEventClick }: { events: EventListItem[]; onEventClick: (id: string) => void }) {
  ...
  const types = [...byType.entries()].sort((a, b) => { const ta = [...a[1].values()].reduce((n, arr) => n + arr.length, 0); const tb = [...b[1].values()].reduce((n, arr) => n + arr.length, 0); return tb - ta; });
  const maxCount = Math.max(...types.map(([, sm]) => [...sm.values()].reduce((n, arr) => n + arr.length, 0)));
  if (drill) {
    const subtypeMap = byType.get(drill.type)!;
    const evts = drill.subtype ? subtypeMap.get(drill.subtype) ?? [] : [...subtypeMap.values()].flat();
    const sorted = [...evts].sort((a, b) => b.date.localeCompare(a.date));
    const label = drill.subtype ? `${TYPE_LABELS[drill.type] ?? drill.type} — ${drill.subtype.replace(/_/g, " ")}` : TYPE_LABELS[drill.type] ?? drill.type;
    return (
      <div>
        <button onClick={() => setDrill(null)} className="flex items-center gap-1.5 text-xs text-neutral-400 hover:text-neutral-700 transition-colors mb-5"><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M9 2L4 7l5 5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>Back</button>
        <div className="flex items-center gap-2 mb-5"><EventTypeIcon type={drill.type} size={14} /><span className="font-serif text-xl text-neutral-900 capitalize">{label}</span><span className="text-sm text-neutral-300">{evts.length}</span></div>
        <div className="space-y-0.5">{sorted.map((e) => <button key={e.id} onClick={() => onEventClick(e.id)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-neutral-50 active:bg-neutral-100 transition-colors text-left group"><span className="text-[10px] text-neutral-300 w-10 flex-shrink-0">{e.date.slice(0, 4)}</span><span className="flex-1 font-serif text-sm text-neutral-900 truncate group-hover:underline underline-offset-2">{e.title}</span><span className="text-[10px] text-neutral-300 flex-shrink-0 truncate max-w-[30%]">{e.venue_name}</span></button>)}</div>
      </div>
    );
  }
  return (
    <div className="space-y-1.5">
      {types.map(([type, subtypeMap]) => {
        ...
        return (
          <div key={type} className="flex items-center gap-3 group">
            <button onClick={() => setDrill({ type, subtype: null })} className="flex items-center gap-1.5 w-24 flex-shrink-0 hover:opacity-70 transition-opacity"><EventTypeIcon type={type} size={12} /><span className="text-[10px] uppercase tracking-widest text-neutral-400 truncate">{TYPE_LABELS[type] ?? type}</span></button>
            <div className="flex-1 flex items-center gap-1.5">
              <div className="flex h-6 rounded-full overflow-hidden" style={{ width: `${barWidth}%`, minWidth: "2rem" }}>
                {subtypes.map(([sub, evts]) => {
                  ...
                  return <button key={sub} title={`${sub.replace(/_/g, " ")} (${evts.length})`} style={{ width: `${pct}%`, background: colorMap.get(sub), opacity: isHovered ? 1 : 0.65, transition: "opacity 0.15s" }} className="h-full focus:outline-none" onMouseEnter={() => setHovered({ type, subtype: sub })} onMouseLeave={() => setHovered(null)} onClick={() => setDrill({ type, subtype: sub })} />;
                })}
              </div>
      ...
      {hovered && (
        <div className="mt-4 text-xs text-neutral-500 flex items-center gap-2">
          <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: colorMap.get(hovered.subtype) }} />
