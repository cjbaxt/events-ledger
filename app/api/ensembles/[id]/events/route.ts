...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [classicalRes, operaRes, balletRes, danceRes] = await Promise.all([
    supabase.from("classical_event").select("event_id").eq("ensemble->id", id),
    supabase.from("opera_event").select("event_id").eq("company->id", id),
    supabase.from("ballet_event").select("event_id").eq("company->id", id),
    supabase.from("dance_event").select("event_id").eq("company->id", id),
  ]);

  const eventIdSet = new Set<string>();
  for (const res of [classicalRes, operaRes, balletRes, danceRes]) {
  ...
  const filtered = (data ?? []).filter((r: Record<string, unknown>) => eventIdSet.has(r.id as string));
  return NextResponse.json(filtered.map((r: Record<string, unknown>) => ({
